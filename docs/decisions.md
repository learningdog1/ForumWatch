...
# 第十九轮决策（2026-09-25，R19：报告推送三连修——AI 简报、富文本渲染、栏目结构）

用户反馈分类行情日报三个问题：①推送里没有 AI 总结；②markdown 没渲染（`#`、`|---|` 表格原样裸奔）；③热点/趋势/预测/行情不明显、挖不出价值。三个根因与裁定：

- **①报告 LLM 直出模式（根因：思考吃光预算 → 空串 → 静默降级模板）**：日报与分类报告的 `provider.chat` 调用此前没带 `disableThinking`（R16 基建已备而未接，语义评估/锐评都接了、唯独报告漏了）。思考型模型思考 token 与正文共用 max_tokens，思考吃光预算时 content 空串 → `LLM returned empty content` → 整体降级纯统计模板照常推送（头注一行不起眼，用户感知即「没带 AI 总结」）。三处调用点（单发直出/分段小结/reduce）全部补 `disableThinking: true`；max_tokens 放宽（分类 2000→4000、日报 1500→3000）适配更长的五段结构。降级路径保留（LLM 失败报告仍不断档），头注加 ⚠️ 更醒目。
- **②推送双形态渲染（根因：sendRaw 纯文本直发，TG 不认 markdown）**：新增 `notify/markdown-report`（纯函数）：报告 markdown → Telegram HTML 子集（标题→粗体、管道表格→剥分隔行+按显示宽度对齐包 `<pre>`、引用块→blockquote、列表→•、`**`/`*`/`` ` ``/`[t](u)` 行内转换、全量转义）与纯文本（去记号、表格同样对齐）两形态。`Notifier.sendRaw(text, opts?)` 增可选 `RawMessageOptions.html`：telegram 非空时以 parse_mode=HTML 发送并禁用链接预览（首 URL 预览卡对报告是噪音），bark/ntfy/webhook 实现不动（单参签名可赋值，用 text 纯文本）。分段上限 3500→`REPORT_CHUNK_MAX`=3000（HTML 标签/转义膨胀留 4096 余量）；转换后 HTML 超 `HTML_SAFE_MAX`=4000 的段退纯文本——绝不因长度上限发送失败。
- **③分类报告升级为「行情简报」五段结构**：prompt 严格锁结构——🧠 总评 / 🔥 热点（按分类归纳、引用真实标题）/ 📈 趋势 / 💰 行情解读（结合价格分位统计解读，无样本如实写）/ 🔮 展望（下期观察点、注明推测）。LLM 载荷新增 `stats`（分类计数/作者榜/同帖多发/价格分位，与确定性渲染段同源 `buildCategoryStats`——行情解读有真实分位数依据且与报告统计段不会两套口径）。确定性栏目 emoji 化：📊 帖量分布 / 🔁 同帖多发与活跃作者 / 💸 价格分位 / 📋 覆盖率（文件与推送一致；推送端按标题粗体渲染，扫一眼分栏）。命中监控日报 prompt 同步结构化（📌 概览 / 📋 命中清单 / 💡 观察）+ 直出模式。附录、map-reduce 体量上限、四联推送判定、tick 调度等 R17 语义全部不动。

**R19b 增补（同日，「现在 AI 一般都是 1M 上下文」）**：

- **60 帖/段固定切分 → token 预算自适应**：旧 `CATEGORY_REPORT_CHUNK=60` 是小上下文时代遗产——679 帖/天的日报要串 13 次调用（任一段失败整体降级，失败概率随段数放大），月报 15k+ 帖直接撞 `30 段 × 60 帖 = 1800` 上限被「体量过大」降级、永远没有 AI 总结。改为 `chunkByTokenBudget`：`estimateTokens`（CJK×1.2 + 其余×0.3，偏保守=偏高——高估只是多几次调用，低估才有撑爆上下文的风险）对每帖载荷估算，按 `REPORT_INPUT_TOKEN_BUDGET=90k` 贪心打包（段开销 8k 先扣；单帖超预算独立成段不裁剪）。预算取 90k 而不是更大：**必须对 128k 上下文模型也安全**——预算超过用户模型上限的失败模式恰是「整份报告没有 AI 总结」（刚修掉的问题换个马甲回来）。实测：679 帖/天 → 1 次调用；15000 帖/月 → 14 段。段数上限 MAX_MAP_CHUNKS=30 保留为时长护栏（30 × 90k ≈ 2.7M token ≈ 数万帖）。
- **超时随载荷放大**：大输入的处理时延显著高于小输入，固定 60s 对 90k-token 载荷偏紧。`timeoutForTokens`：60s 底数 + 1s/1k token，封顶 180s（串行 30 段封顶 90 分钟，月报可接受）。
- **载荷不再省料（带 url + 摘要）**：每帖附 `url` 与 `excerpt`（截 120 字，来源提供时）；prompt 允许 🔥 热点条目附代表帖 markdown 链接、命中日报清单逐条 `[标题](url)`，并严格限定 url 原样取自载荷禁止编造。渲染端（markdown-report）相应增加裸 URL 显式链接化——HTML parse_mode 下客户端对正文裸 URL 的自动识别不可依赖，显式 `<a>` 才稳。降级模板命中行行尾也附裸 url。
