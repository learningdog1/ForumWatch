 ...
 * → 尝试推送（单个失败不中断本轮）→ 轮末 flush/prune/持久化 totalHits。
 *
...
export class MonitorEngine {
   ...
   * 推送失败（TelegramError 等）只记 notifyError，不中断本轮后续 topic。
   */
  private async processHit(
    ...
    const hit: HitRecord = { topic, matchedKeywords, notifiedAt, notifyError }
    ...
    if (notifyError !== null) {
      this.deps.logger.error(
        `notify failed for topic ${topic.id} "${topic.title}": ${notifyError}`
      )
  ...
  private finishFailure(err: unknown, cfg: AppConfig): void {
    ...
    const backoffSec =
      computeBackoffMs(this.status.consecutiveFailures, cfg.pollIntervalSec * 1000) / 1000
