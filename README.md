...
常驻系统托盘的论坛监控工具（v0.2 起：多论坛架构 + AI 能力，前身 NodeSeek Monitor）：按设定间隔轮询论坛新帖，标题命中关键词（字面匹配）或符合你的兴趣描述（AI 语义匹配）时立即推送到 Telegram；还可让 AI 每天定点把当天命中总结成日报。当前内置 [NodeSeek](https://www.nodeseek.com) 一个来源，架构上为接入更多论坛预留。macOS / Windows 双平台。
