...
你关注的论坛（NodeSeek、V2EX、或任意 RSS 论坛）一出新帖，只要命中你设的关键词、价格条件，或被 AI 判定与你有关，就会立刻推送到你的 Telegram / iOS / 任意自建通道。全程本机运行，关窗不退。
...
- **推送给你，推到你习惯的地方**：Telegram、Bark（iOS）、ntfy、Webhook 多通道并发，可配路由规则按来源 / 命中方式分流；支持摘要攒批与免打扰时段。
...
4. **接推送**：设置 → 推送通道，至少配一条。以 Telegram 为例：
   ...
   不用 Telegram 也行：Bark 填设备 Key、ntfy 填 topic、Webhook 填你的接收地址，支持同时配多条通道。
5. **点「发送测试消息」**，全部就绪通道各收到一条即成功。之后窗口可以关掉，托盘继续盯论坛。
...
**收不到推送？** 点设置页的「发送测试消息」：全都收不到 → 检查关键词是否命中过标题、bot 是否被你先发过消息；单条通道收不到 → 该通道凭据 / 网络问题，「历史命中」里每条记录都带逐通道送达明细。更细的排查矩阵见 [深度文档](docs/usage.md#故障排查矩阵)。
