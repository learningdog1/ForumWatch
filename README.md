...
- **盯住任何论坛**：内置 NodeSeek 与 V2EX；任意提供 RSS 2.0 / Atom 的论坛填地址即可接入，设置页有 Linux.do、LowEndTalk（全站）、LowEndTalk Offers（只推优惠帖，feed 相同 + 分类白名单过滤）一键预设。每个来源还能单独覆盖关键词 / 匹配模式 / AI 兴趣 / 语义阈值（该行「匹配」面板，留空跟全局），或开**全匹配**——该来源新帖全部直推、零 AI 调用（排除词与分类过滤仍生效）。每个来源独立健康状态，一个被拦不拖累其他。
- **三种方式决定"什么算值得推送"**，可叠加：
  - **关键词**：包含词条任一命中即推，排除词一票否决，大小写不敏感；词条内用 `&&` 连多个词（如 `搬瓦工 && 香港`）表示须**同时命中**才推——「(A 且 B) 或 C」这类组合条件也能表达。
  - **价格规则**：从标题提取「周期 / 价格 / 流量」（如"年付 ¥99、500G 流量"），按你的条件（如"年付 + ≤¥100 + ≥500G"）精确比对——找 VPS 羊毛的利器，确定性命中、零成本。
  - **AI 语义匹配**（可选）：配置任意 OpenAI 兼容模型（内置 DeepSeek / Kimi / GLM 预设），用自然语言写兴趣（"Oracle 免费 ARM 的羊毛""年付 100 元以内的低价小鸡"），AI 逐帖判断。「字面 + 语义」模式下关键词命中直接推送、零 AI 调用，未命中的才进 AI。还能在命中行点 👍/👎 反馈，越用越合口味；上游限流或故障时指数退避（冷却期零调用），连续未决的帖自动降级字面判定，不刷调用、不丢帖；没配 AI 自动按关键词监控，不中断。
- **推送到你的 Telegram**：命中即推，可附一句 AI 锐评；作者名可点击直达对方主页（私有帖也知道是谁发的、一键联系），命中词带 `#` 标签、点一下就能筛选同关键词的推送；支持摘要攒批与免打扰时段，还能用 Bot 远程控制（查状态 / 启停 / 改配置）。
- **不只是一条通知**：AI 可给每条命中附一句锐评；每天定点把当天命中总结成日报推给你。
- **分类阶段性总结（日 / 周 / 月报）**：不止盯着命中——引擎把观察到的**每一个新帖**（命中与否都算）落进本地全量话题存档（保留 35 天），按分类（默认情报 / 交易 / 测评）定时产出三档行情总结：AI 总述 + 分类热点矩阵 + 趋势（同帖多发 / 活跃作者）+ VPS 价格行情分位表，报告文件附全量帖子清单可核对防漏（推送不附，防月报刷屏）。AI 不可用时自动降级为纯统计模板，报告不断档。周报固定周一总结上一完整周、月报固定每月 1 日总结上一自然月。
...
README 只负责让你决定要不要用。每一项行为的准确语义——关键词与价格规则的提取口径、AI 配置与限额、推送路由与重试、处置流水的 14 类出口、分类总结报告的存档与覆盖口径、headless 直跑模式、已知限制清单——全部在 **[深度文档 docs/usage.md](docs/usage.md)**，与代码实现一一对应。
